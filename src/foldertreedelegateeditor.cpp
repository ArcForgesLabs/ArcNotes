...
FolderTreeDelegateEditor::FolderTreeDelegateEditor(QTreeView* view, const QStyleOptionViewItem& option,
    ...
    m_folderIcon->setText(QStringLiteral("\ue2c7"));  // folder
    ...
    m_contextButton->setText(QStringLiteral("\uf141"));  // fa-ellipsis-h
...
void FolderTreeDelegateEditor::updateDelegate() {
    ...
    if (m_view->selectionModel()->isSelected(m_index)) {
        ...
        labelStyle = QStringLiteral("QLabel{color: rgb(%1, %2, %3);}")
                         .arg(QString::number(m_titleColor.red()), QString::number(m_titleColor.green()),
                              QString::number(m_titleColor.blue()));
        folderIconStyle = QStringLiteral("QPushButton{border: none; padding: 0px; color: rgb(%1, %2, %3);}")
                              .arg(QString::number(m_folderIconColor.red()), QString::number(m_folderIconColor.green()),
    ...
    if (m_index.data(NodeItem::Roles::IsExpandable).toBool()) {
        if (m_view->isExpanded(m_index)) {
            m_expandIcon->setText(QStringLiteral("\uf078"));  // fa-chevron-down
        } else {
            m_expandIcon->setText(QStringLiteral("\uf054"));  // fa-chevron-right
        }
    }
}
...
void FolderTreeDelegateEditor::paintEvent(QPaintEvent* event) {
    ...
    if (m_view->selectionModel()->isSelected(m_index)) {
        ...
        const auto* listView = static_cast<NoteListView*>(m_listView);
