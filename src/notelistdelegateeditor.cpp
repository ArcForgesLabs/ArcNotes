...
NoteListDelegateEditor::NoteListDelegateEditor(const NoteListDelegate* delegate, NoteListView* view,
                                               ...
                                               TagPool* tagPool, QWidget* parent)
    : QWidget(parent),
      m_delegate(delegate),
      m_option(option),
    ...
    if (m_delegate->isInAllNotes()) {
        ...
        if (noteListModel != nullptr) {
            ...
            if (noteListModel->hasPinnedNote() &&
                (noteListModel->isFirstPinnedNote(idx) || noteListModel->isFirstUnpinnedNote(idx))) {
                y += 25;
            }
        }
        int fourthYOffset = 0;
        ...
        int fifthYOffset = 0;
        if ((noteListModel != nullptr) && noteListModel->hasPinnedNote() && !m_view->isPinnedNotesCollapsed() &&
    ...
    connect(m_tagListView->verticalScrollBar(), &QScrollBar::valueChanged, this, [this] {
        ...
        static_cast<NoteListModel*>(m_view->model())
...
void NoteListDelegateEditor::paintBackground(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    if (m_view->isDragging() && !isPinned && !m_view->isDraggingInsidePinned()) {
        if ((noteListModel != nullptr) && noteListModel->isFirstUnpinnedNote(index) &&
            (index.row() == (noteListModel->rowCount() - 1))) {
            auto rect = bufferRect;
            rect.setHeight(4);
            bufferPainter.fillRect(rect, QBrush("#d6d5d5"));
            rect = bufferRect;
    ...
    if ((noteListModel != nullptr) && (m_delegate != nullptr) &&
        m_delegate->shouldPaintSeparator(index, *noteListModel)) {
    ...
    if (noteListModel->hasPinnedNote()) {
        if (noteListModel->isFirstPinnedNote(index)) {
            QRect headerRect(rowPosX + (note_list_constants::LEFT_OFFSET_X / 2), rowPosY,
                             rowWidth - (note_list_constants::LEFT_OFFSET_X / 2), 25);
#ifdef __APPLE__
...
QPixmap NoteListDelegateEditor::renderToPixmap() {
    ...
    const auto idx = static_cast<NoteListModel*>(m_view->model())->getNoteIndex(m_id);
...
void NoteListDelegateEditor::resizeEvent(QResizeEvent* event) {
    ...
    if (m_delegate->isInAllNotes()) {
        int y = 90;
        const auto* noteListModel = static_cast<NoteListModel*>(m_view->model());
        const auto idx = noteListModel->getNoteIndex(m_id);
        if (noteListModel->hasPinnedNote() &&
            (noteListModel->isFirstPinnedNote(idx) || noteListModel->isFirstUnpinnedNote(idx))) {
            y += 25;
        }
        ...
        int fifthYOffset = 0;
        if (noteListModel->hasPinnedNote() && !m_view->isPinnedNotesCollapsed() &&
            noteListModel->isFirstUnpinnedNote(idx)) {
            fifthYOffset = note_list_constants::LAST_PINNED_TO_UNPINNED_HEADER;
        }
        int yOffsets = fourthYOffset + fifthYOffset;
        y += yOffsets;

        m_tagListView->setGeometry(note_list_constants::LEFT_OFFSET_X - 5, y + 5, rect().width() - 15,
        ...
        m_tagListView->setGeometry(note_list_constants::LEFT_OFFSET_X - 5, y, rect().width() - 15,
                                   m_tagListView->height());
    }
    recalculateSize();
}
