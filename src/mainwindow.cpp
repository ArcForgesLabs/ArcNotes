...
Q_LOGGING_CATEGORY(lc_editor_settings_qml, "arcnotes.qml.editor_settings")
...
#ifdef __APPLE__
      m_displayFont(QFont(QStringLiteral("SF Pro Text")).exactMatch() ? QStringLiteral("SF Pro Text")
...
void MainWindow::resizeAndPositionEditorSettingsWindow() {
    ...
    m_editorSettingsWidget->move(buttonGlobalPos.x() - m_editorSettingsWidget->width() + m_dotsButton->width(),
...
void MainWindow::setupMainWindow() {
    ...
    if (mainWindowStyleFile.open(QFile::ReadOnly)) {
        m_styleSheet = QString::fromLatin1(mainWindowStyleFile.readAll());
    } else {
        qWarning() << "Failed to open stylesheet" << mainWindowStyleFile.fileName();
        m_styleSheet.clear();
    }
    ...
    if (scollBarStyleFile.open(QFile::ReadOnly)) {
        ...
        qWarning() << "Failed to open stylesheet" << scollBarStyleFile.fileName();
    }
...
void MainWindow::setupKeyboardShortcuts() {
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_L), this), &QShortcut::activated, this,
            [this]() { m_listView->setFocus(); });
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_S), this), &QShortcut::activated, this, [this]() {
#if QT_VERSION >= QT_VERSION_CHECK(6, 2, 0)
...
#endif
    connect(shortcut, &QxtGlobalShortcut::activated, this, [this]() {
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 2, 0)
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_K), this), &QShortcut::activated, this, [this]() {
        if (m_kanbanWidget->isHidden()) {
...
void MainWindow::setupButtons() {
    ...
    m_globalSettingsButton->setText(QStringLiteral("\uf013"));  // fa-gear
    ...
    m_dotsButton->setText(QStringLiteral("\ue5d3"));  // ellipsis_h
    ...
    m_switchToKanbanViewButton->setText(QStringLiteral("\ueb7f"));  // view_kanban
    m_switchToTextViewButton->setFont(materialSymbols);
    m_switchToTextViewButton->setText(QStringLiteral("\uef42"));  // article

    materialSymbols.setPointSize(20 + pointSizeOffset);
    m_toggleTreeViewButton->setFont(materialSymbols);
    if (m_foldersWidget->isHidden()) {
        m_toggleTreeViewButton->setText(QStringLiteral("\ue31c"));  // keyboard_tab_rtl
    } else {
        m_toggleTreeViewButton->setText(QStringLiteral("\uec73"));  // keyboard_tab_rtl
    }
    ...
    m_newNoteButton->setText(QStringLiteral("\uf067"));  // fa_plus
...
void MainWindow::setupSignalsSlots() {
    ...
    connect(&m_updater, &UpdaterWindow::dontShowUpdateWindowChanged, this,
            [this](bool state) { m_dontShowUpdateWindow = state; });
#endif
    // actions
...
#if !defined(Q_OS_MAC)
    connect(m_textEdit->verticalScrollBar(), &QAbstractSlider::actionTriggered, this, [this]() {
    ...
    connect(m_textEditScrollBarTimer, &QTimer::timeout, this, [this]() { m_textEdit->verticalScrollBar()->hide(); });
#endif

    // MainWindow <-> DBManager
    connect(this, &MainWindow::requestNodesTree, m_dbManager, &DBManager::onNodeTagTreeRequested,
            Qt::BlockingQueuedConnection);
    connect(this, &MainWindow::requestRestoreNotes, m_dbManager, &DBManager::onRestoreNotesRequested,
            Qt::BlockingQueuedConnection);
    ...
    connect(this, &MainWindow::requestMigrateTrashFromV0_9_0, m_dbManager, &DBManager::onMigrateTrashFrom0_9_0Requested,
            Qt::BlockingQueuedConnection);
    ...
    connect(m_listViewLogic, &ListViewLogic::setNewNoteButtonVisible, this,
            [this](bool visible) { m_ui->newNoteButton->setVisible(visible); });
    ...
    connect(this, &MainWindow::requestChangeDatabasePath, m_dbManager, &DBManager::onChangeDatabasePathRequested,
            Qt::QueuedConnection);
...
void MainWindow::setupSearchEdit() {
    ...
    m_clearButton->setText(QStringLiteral("\uf057"));  // fa-circle-xmark
    ...
    m_searchButton->setText(QStringLiteral("\uf002"));  // fa-magnifying-glass
...
void MainWindow::setupEditorSettings() {
    ...
    m_editorSettingsQuickView.engine()->addImportPath(qmlImportPath);
    qCInfo(lc_editor_settings_qml) << "EditorSettings import path:" << qmlImportPath;
    ...
    QObject::connect(&m_editorSettingsQuickView, &QQuickView::statusChanged, this, [this](QQuickView::Status status) {
        qCInfo(lc_editor_settings_qml) << "EditorSettings status changed:" << status;
        if (status == QQuickView::Error) {
            for (const auto& error : m_editorSettingsQuickView.errors()) {
                qCWarning(lc_editor_settings_qml) << error;
            }
        }
    });
    m_editorSettingsQuickView.setSource(source);
    if (m_editorSettingsQuickView.status() == QQuickView::Error) {
        qCWarning(lc_editor_settings_qml) << "Failed to load EditorSettings.qml";
        for (const auto& error : m_editorSettingsQuickView.errors()) {
            qCWarning(lc_editor_settings_qml) << error;
        }
    } else if (auto* root_object = m_editorSettingsQuickView.rootObject()) {
        qCInfo(lc_editor_settings_qml) << "EditorSettings root object:" << root_object->metaObject()->className()
                                       << "size=" << root_object->property("width").toDouble() << 'x'
                                       << root_object->property("height").toDouble()
                                       << "visible=" << root_object->property("visible").toBool();
    } else {
        qCWarning(lc_editor_settings_qml) << "EditorSettings root object is null";
    }
    ...
    QJsonObject dataToSendToView{{"displayFont", QFont(QStringLiteral("SF Pro Text")).exactMatch()
...
#elif _WIN32
    if (QFont("Calibri").exactMatch()) m_listOfSansSerifFonts.push_front("Calibri");

    if (QFont("Arial").exactMatch()) m_listOfSansSerifFonts.push_front("Arial");

    if (QFont("Segoe UI").exactMatch()) m_listOfSansSerifFonts.push_front("Segoe UI");
#endif
...
void MainWindow::setupKanbanView() {
    ...
    m_kanbanWidget->engine()->addImportPath(QCoreApplication::applicationDirPath() + QStringLiteral("/qml"));
    ...
    m_kanbanWidget->setSource(source);
    if (m_kanbanWidget->status() == QQuickWidget::Error) {
        qWarning() << "Failed to load kanbanMain.qml";
        for (const auto& error : m_kanbanWidget->errors()) {
 ...
 */
void MainWindow::setActivationSuccessful(const QString& licenseKey, bool removeGracePeriodStartedDate) {
...
void MainWindow::getSubscriptionStatus() {
    QString validateLicenseEndpoint =
        m_paymentDetails["purchaseApiBase"].toString() + m_paymentDetails["validateLicenseEndpoint"].toString();
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            ...
            if (response.isEmpty()) {
                ...
                if (!m_localLicenseData->contains(QStringLiteral("gracePeriodStartedDate"))) {
                    ...
                    QDateTime gracePeriodStartedDate =
                        m_localLicenseData
                            ->value(QStringLiteral("gracePeriodStartedDate"), QDateTime::currentDateTime())
                            .toDateTime();
                    ...
                    if (QDateTime::currentDateTime() > dateAfterSevenDays) {
                        // Show grace period is over window, you need internet connection
                       ...
                       response[QStringLiteral("license_key")].toObject()[QStringLiteral("status")].toString() ==
                           "expired") {
            ...
            if (response.contains("license_key") && response.contains("valid") && response["valid"] == true) {
                if (response[QStringLiteral("license_key")].toObject()[QStringLiteral("status")].toString() ==
                    "inactive") {
                    int activationUsage =
                        response[QStringLiteral("license_key")].toObject()[QStringLiteral("activation_usage")].toInt();
                    int activationLimit =
                        response[QStringLiteral("license_key")].toObject()[QStringLiteral("activation_limit")].toInt();
                    if (activationUsage >= activationLimit) {
                        ...
                        QString activateLicenseEndpoint = m_paymentDetails["purchaseApiBase"].toString() +
                ...
                } else if (response[QStringLiteral("license_key")].toObject()[QStringLiteral("status")].toString() ==
                           "active") {
                    // License is active
        ...
        if (!m_isProVersionActivated && showSubscriptionWindowWhenNotPro) m_subscriptionWindow->show();
    });
}
...
void MainWindow::setupDatabases() {
    m_settingsDatabase = new QSettings(QSettings::IniFormat, QSettings::UserScope, QStringLiteral("Awesomeness"),
                                       QStringLiteral("Settings"), this);

#if !defined(PRO_VERSION)
#if defined(Q_OS_UNIX) && !defined(Q_OS_MACOS)
    m_localLicenseData = new QSettings(QSettings::NativeFormat, QSettings::UserScope, QStringLiteral("Awesomeness"),
                                       QStringLiteral(".notesLicenseData"), this);
#else
    m_localLicenseData = new QSettings(QSettings::NativeFormat, QSettings::UserScope, QStringLiteral("Awesomeness"),
                                       QStringLiteral("notesLicenseData"), this);
#endif
    ...
    if (!folderCreated) qFatal("ERROR: Can't create settings folder : %s", dir.absolutePath().toStdString().c_str());
    ...
    if (!QFile::exists(noteDBFilePath)) {
        ...
        if (!noteDBFile.open(QIODevice::WriteOnly)) qFatal("ERROR : Can't create database file");
    ...
    connect(m_dbThread, &QThread::started, this, [this, noteDBFilePath, doCreate, needMigrateFromV1_5_0, dir]() {
    ...
    connect(this, &MainWindow::requestOpenDBManager, m_dbManager, &DBManager::onOpenDBManagerRequested,
            Qt::QueuedConnection);
    connect(this, &MainWindow::requestMigrateNotesFromV1_5_0, m_dbManager, &DBManager::onMigrateNotesFrom1_5_0Requested,
            Qt::QueuedConnection);
...
void MainWindow::setupModelView() {
    ...
    m_listViewLogic =
        new ListViewLogic(m_listView, m_listModel, m_searchEdit, m_clearButton, m_tagPool, m_dbManager, m_listView);
    m_treeView = m_ui->treeView;
    m_treeView->setModel(m_treeModel);
    m_treeViewLogic = new TreeViewLogic(m_treeView, m_treeModel, m_dbManager, m_listView, this);
#if QT_VERSION >= QT_VERSION_CHECK(6, 2, 0)
    m_noteEditorLogic = new NoteEditorLogic(m_textEdit, m_editorDateLabel, m_searchEdit, m_kanbanWidget,
                                            m_ui->tagListView, m_tagPool, m_dbManager, this);
    m_kanbanWidget->rootContext()->setContextProperty("noteEditorLogic", m_noteEditorLogic);
#else
    m_noteEditorLogic = new NoteEditorLogic(m_textEdit, m_editorDateLabel, m_searchEdit, m_ui->tagListView, m_tagPool,
                                            m_dbManager, this);
#endif
    m_editorSettingsQuickView.rootContext()->setContextProperty("noteEditorLogic", m_noteEditorLogic);
}
...
void MainWindow::restoreStates() {
    ...
    setUseNativeWindowFrame(
        m_settingsDatabase->value(QStringLiteral("useNativeWindowFrame"), nativeByDefault).toBool());
    ...
    auto expandedFolder =
        m_settingsDatabase->value(QStringLiteral("currentExpandedFolder"), QStringList{}).toStringList();
    ...
    auto currentSelectTagsId =
        m_settingsDatabase->value(QStringLiteral("currentSelectTagsId"), QStringList{}).toStringList();
    QSet<int> tags;
    ...
    auto currentSelectNotes =
        m_settingsDatabase->value(QStringLiteral("currentSelectNotesId"), QStringList{}).toStringList();
    QSet<int> notesId;
 ...
 * Removes applied formatting: bold, italic, strikethrough, heading of the current line
 ...
 * \param formatChars
 * Removes applied formatting: bold, italic, strikethrough
 */
...
void MainWindow::setupGlobalSettingsMenu() {
    ...
    connect(autostartAction, &QAction::triggered, this,
            [this, autostartAction]() { m_autostart.setAutostart(autostartAction->isChecked()); });
    ...
    connect(changeDBPathAction, &QAction::triggered, this, [this]() {
    ...
    connect(aboutAction, &QAction::triggered, this, [this]() { m_aboutWindow.show(); });

    m_mainMenu.addSeparator();

#if !defined(PRO_VERSION)
    // Buy/Manage subscription
    m_buyOrManageSubscriptionAction =
        m_mainMenu.addAction(tr(m_isProVersionActivated ? "&Notes Pro (Paid)" : "&Buy Notes Pro..."));
...
void MainWindow::changeEditorFontTypeFromStyleButtons(FontTypeface::Value fontTypeface, int chosenFontIndex) {
    if (chosenFontIndex < 0) return;

    switch (fontTypeface) {
        case FontTypeface::Mono:
            if (chosenFontIndex > m_listOfMonoFonts.size() - 1) return;
            m_chosenMonoFontIndex = chosenFontIndex;
            break;
        case FontTypeface::Serif:
            if (chosenFontIndex > m_listOfSerifFonts.size() - 1) return;
            m_chosenSerifFontIndex = chosenFontIndex;
            break;
        case FontTypeface::SansSerif:
            if (chosenFontIndex > m_listOfSansSerifFonts.size() - 1) return;
    ...
    if (!m_noteEditorLogic->isTempNote()) {
        ...
        if ((!inf.isInTag) && (inf.parentFolderId > ROOT_FOLDER_ID)) {
            ...
            QMetaObject::invokeMethod(m_dbManager, "getNode", Qt::BlockingQueuedConnection,
                                      Q_RETURN_ARG(NodeData, parent), Q_ARG(int, inf.parentFolderId));
...
void MainWindow::quitApplication() {
    ...
    m_settingsDatabase->setValue(QStringLiteral("isTextFullWidth"),
                                 m_textEdit->lineWrapMode() == QTextEdit::WidgetWidth);
...
void MainWindow::restoreNotesFile() {
    if (m_listModel->rowCount() > 0) {
        QMessageBox msgBox;
        msgBox.setText(
            tr("Warning: All current notes will be lost. Make sure to create a backup "
...
void MainWindow::executeImport(const bool replace) {
    QString fileName =
        QFileDialog::getOpenFileName(this, tr("Open Notes Backup File"), "", tr("Notes Backup File (*.nbk)"));
    if (fileName.isEmpty()) {
        return;
    }
    QFile file(fileName);
...
void MainWindow::exportNotesFile() {
    QString fileName =
        QFileDialog::getSaveFileName(this, tr("Save Notes"), "notes.nbk", tr("Notes Backup File (*.nbk)"));
    if (fileName.isEmpty()) {
        return;
    }
    QFile file(fileName);
...
void MainWindow::collapseFolderTree() {
    m_toggleTreeViewButton->setText(QStringLiteral("\ue31c"));  // keyboard_tab
    ...
    if (!m_canMoveWindow && !isMaximized() && !isFullScreen()) {
        switch (m_stretchSide) {
            ...
            case StretchSide::BottomRight:
                ...
                break;
            default:
                if (!m_canStretchWindow) m_ui->centralWidget->setCursor(Qt::ArrowCursor);
                break;
        }
    }
#endif
...
void MainWindow::dropShadow(QPainter& painter, ShadowType type, MainWindow::ShadowSide side) {
    ...
    QRect outerRect(innerRect.x() - resizedShadowWidth, innerRect.y() - resizedShadowWidth,
                    innerRect.width() + (2 * resizedShadowWidth), innerRect.height() + (2 * resizedShadowWidth));
    ...
    switch (event->type()) {
        ...
        case QEvent::HoverEnter: {
            if (object == m_textEdit->verticalScrollBar()) {
                ...
                if (isSearching) m_textEdit->setFocusPolicy(Qt::NoFocus);
            }
            break;
        }
...
void MainWindow::setMargins(QMargins margins) {
    if (m_useNativeWindowFrame) return;
...
bool MainWindow::isTitleBar(int x, int y) const {
    if (m_useNativeWindowFrame) return false;
