...
bool DBManager::updateNoteContent(const NodeData& note) {
    ...
    if (!query.prepare(
            QStringLiteral("UPDATE node_table SET modification_date = :modification_date, content = :content, "
...
QList<NodeData> DBManager::readOldNBK(const QString& fileName) {
    QList<NodeData> noteList;
    {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            qWarning() << "Failed to open backup file" << fileName;
        }
    }
    if (noteList.isEmpty()) {
        QFile file(fileName);
...
void DBManager::moveNode(int nodeId, const NodeData& target) {
    ...
    if (target.id() == TRASH_FOLDER_ID) {
        ...
        if (!query.prepare(
                QStringLiteral("UPDATE node_table SET parent_id = :parent_id, absolute_path = :absolute_path, "
    ...
    if (node.nodeType() == NodeData::Type::Folder) {
        ...
        for (const auto id : children.keys()) {
            ...
            if (target.id() == TRASH_FOLDER_ID) {
                qint64 deletionTime = QDateTime::currentMSecsSinceEpoch();
                if (!query.prepare(
                        QStringLiteral("UPDATE node_table SET absolute_path = :absolute_path, is_pinned_note = "
...
void DBManager::searchForNotes(const QString& keyword, const ListViewInfo& inf) {
    ...
    if (!inf.isInTag && inf.parentFolderId == ROOT_FOLDER_ID) {
        ...
        QVector<QSet<int>> noteIdSets;
        for (const auto& tagId : inf.currentTagList) {
            QSet<int> noteIdsForTag;
            ...
            if (status) {
                while (query.next()) {
                    noteIdsForTag.insert(query.value(0).toInt());
                }
            } else {
                qDebug() << __FUNCTION__ << __LINE__ << query.lastError();
            }
            noteIdSets.append(noteIdsForTag);
        }
        if (noteIdSets.empty()) {
        ...
        QSet<int> noteIds;
        int smallestSetIndex = 0;
        for (int i = 1; i < noteIdSets.size(); ++i) {
            if (noteIdSets[i].size() < noteIdSets[smallestSetIndex].size()) {
                smallestSetIndex = i;
            }
        }
        for (const int id : std::as_const(noteIdSets[smallestSetIndex])) {
            bool ok = true;
            for (int i = 0; i < noteIdSets.size(); ++i) {
                if (i == smallestSetIndex) {
                    continue;
                }
                if (!noteIdSets[i].contains(id)) {
                    ok = false;
                }
            }
            if (ok) {
...
void DBManager::onImportNotesRequested(const QString& fileName) {
    ...
    const auto magicHeader = file.read(16);
    file.close();
    if (QString::fromUtf8(magicHeader).startsWith(QStringLiteral("SQLite format 3"))) {
        ...
        QMap<int, int> tagIdMap;
        {
            ...
            std::sort(tagList.begin(), tagList.end(),
                      [](const auto& a, const auto& b) { return a.relativePosition() < b.relativePosition(); });
        ...
        }
        {
            ...
            if (status) {
                const auto* functionName = Q_FUNC_INFO;
                while (outQuery.next()) {
                    NodeData node;
                ...
                auto matchFolderFunc = [&](int id, const auto& matchFolderFunctor) {
                    if (!nodeList.contains(id)) {
                        qDebug() << functionName << __LINE__ << "node not found";
                        return;
                    }
                    ...
                    if (folderIdMap.contains(node.parentId())) {
                        ...
                        if (!query.prepare(
                            ...
                            qDebug() << functionName << __LINE__ << query.lastError();
                        }
                ...
                if (rootFolder != nullptr) {
                    ...
                    auto sortChildFunc = [&](Folder* f) {
                        std::sort(f->children.begin(), f->children.end(), [&](const Folder* f1, const Folder* f2) {
...
void DBManager::exportNotes(const QString& baseExportPath, const QString& extension) {
    ...
    while (directory.exists(exportPathNew)) {
        exportPathNew =
            QStringLiteral("%1%2%3 %4").arg(baseExportPath, QDir::separator(), rootFolderName).arg(counter++);
    }
    ...
    while (query.next()) {
        ...
        if (safeTitle.contains("<br />")) safeTitle = safeTitle.section("<br />", 0, 0, QString::SectionSkipEmpty);
        ...
        while (directory.exists(filePath)) {
            filePath = QStringLiteral("%1%2%3 %4%5")
                           .arg(notePath, QDir::separator(), safeTitle, QString::number(counter++), extension);
        }
