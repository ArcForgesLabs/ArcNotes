...
public:
#if QT_VERSION >= QT_VERSION_CHECK(6, 2, 0)
    explicit NoteEditorLogic(CustomDocument* textEdit, QLabel* editorDateLabel, QLineEdit* searchEdit,
                             QWidget* kanbanWidget, TagListView* tagListView, TagPool* tagPool, DBManager* dbManager,
    ...
    [[nodiscard]] int currentAdaptableEditorPadding() const;
    void setCurrentAdaptableEditorPadding(int newCurrentAdaptableEditorPadding);

    [[nodiscard]] int currentMinimumEditorPadding() const;
...
private:
    ...
    QString moveTextToNewLinePosition(const QString& inputText, int startLinePosition, int endLinePosition,
                                      int newLinePosition, bool isColumns = false);
