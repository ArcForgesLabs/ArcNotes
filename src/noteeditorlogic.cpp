...
NoteEditorLogic::NoteEditorLogic(CustomDocument* textEdit, QLabel* editorDateLabel, QLineEdit* searchEdit,
                                 QWidget* kanbanWidget, TagListView* tagListView, TagPool* tagPool,
                                 DBManager* dbManager, QObject* parent)
#else
NoteEditorLogic::NoteEditorLogic(CustomDocument* textEdit, QLabel* editorDateLabel, QLineEdit* searchEdit,
                                 TagListView* tagListView, TagPool* tagPool, DBManager* dbManager, QObject* parent)
#endif
    : QObject(parent),
      m_textEdit{textEdit},
...
void NoteEditorLogic::showNotesInEditor(const QVector<NodeData>& notes) {
    ...
    if (notes.size() == 1 && notes[0].id() != INVALID_NODE_ID) {
        ...
        m_textEdit->clear();
        auto padding = m_currentAdaptableEditorPadding > m_currentMinimumEditorPadding ? m_currentAdaptableEditorPadding
...
void NoteEditorLogic::updateTaskText(int startLinePosition, int endLinePosition, const QString& newText) {
    ...
    if (block.isValid()) {
        ...
        // the parser - interpret the task's description as columns
...
void NoteEditorLogic::removeColumn(int startLinePosition, int endLinePosition) {
    ...
    if (startLinePosition < 0 || endLinePosition < startLinePosition) return;
    QTextDocument* document = m_textEdit->document();
    QTextCursor cursor(document);
    cursor.setPosition(document->findBlockByNumber(startLinePosition).position());
...
void NoteEditorLogic::appendNewColumn(QJsonArray& data, QJsonObject& currentColumn, QString& currentTitle,
                                      QJsonArray& tasks) {
    if (!tasks.isEmpty()) {
...
void NoteEditorLogic::highlightSearch() const {
    ...
    while (m_textEdit->find(searchString)) extraSelections.append({m_textEdit->textCursor(), highlightFormat});
