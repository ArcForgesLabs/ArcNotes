...
#elif _WIN32
    QFont fontToUse =
        QFont(QStringLiteral("Segoe UI")).exactMatch() ? QStringLiteral("Segoe UI") : QStringLiteral("Roboto");
#else
...
#ifdef UseXdgOpen
    if (XDGOPEN_PROCESS.isOpen()) XDGOPEN_PROCESS.close();
#endif
...
void UpdaterWindow::resetControls() {
    ...
    if (m_ui->changelog->toPlainText().isEmpty()) {
        ...
        m_ui->changelog->setText(changelogText.append("\n"));  // Don't know why currently changelog box is disappearing
                                                               // at the bottom, so I add a new line to see the text.
    }
...
void UpdaterWindow::startDownload(const QUrl& url) {
    ...
    if (m_fileName.isEmpty()) {
        m_fileName = QStringLiteral("%1_Update_%2.bin")
                         .arg(QCoreApplication::applicationName(), m_updater->getLatestVersion(UPDATES_URL));
    }

    /* Prepare download directory */
    if (!m_downloadDir.exists()) m_downloadDir.mkpath(".");
...
void UpdaterWindow::calculateTimeRemaining(qint64 received, qint64 total) {
    ...
    if (difference > 0) {
        ...
        if (timeRemaining > 7200) {
            timeRemaining /= 3600;
            int hours = static_cast<int>(std::lround(timeRemaining));
            ...
            timeRemaining /= 60;
            int minutes = static_cast<int>(std::lround(timeRemaining));
            ...
            int seconds = static_cast<int>(std::lround(timeRemaining));
...
void UpdaterWindow::mousePressEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton) {
        if (event->position().x() < width() - 5 && event->position().x() > 5 &&
            event->position().toPoint().y() < height() - 5 && event->position().toPoint().y() > 5) {
