...
TreeViewLogic::TreeViewLogic(NodeTreeView* treeView, NodeTreeModel* treeModel, DBManager* dbManager,
    ...
    connect(this, &TreeViewLogic::requestChangeTagColorInDB, m_dbManager, &DBManager::changeTagColor,
            Qt::QueuedConnection);
    ...
    connect(m_treeModel, &NodeTreeModel::requestUpdateNodeRelativePosition, m_dbManager, &DBManager::updateRelPosNode,
            Qt::QueuedConnection);
    connect(m_treeModel, &NodeTreeModel::requestUpdateTagRelativePosition, m_dbManager, &DBManager::updateRelPosTag,
            Qt::QueuedConnection);
...
void TreeViewLogic::loadTreeModel(const NodeTagTreeData& treeData) {
    m_treeModel->setTreeData(treeData);
    {
        ...
        QMetaObject::invokeMethod(m_dbManager, "getChildNotesCountFolder", Qt::BlockingQueuedConnection,
                                  Q_RETURN_ARG(NodeData, node), Q_ARG(int, ROOT_FOLDER_ID));
    ...
    }
    {
        NodeData node;
        QMetaObject::invokeMethod(m_dbManager, "getChildNotesCountFolder", Qt::BlockingQueuedConnection,
                                  Q_RETURN_ARG(NodeData, node), Q_ARG(int, TRASH_FOLDER_ID));
...
void TreeViewLogic::onAddFolderRequested(bool fromPlusButton) {
    ...
    if (parentId != ROOT_FOLDER_ID) {
        hs[NodeItem::Roles::AbsPath] = currentIndex.data(NodeItem::Roles::AbsPath).toString() + PATH_SEPARATOR +
                                       QString::number(newlyCreatedNodeId);
        ...
        hs[NodeItem::Roles::AbsPath] =
            PATH_SEPARATOR + QString::number(ROOT_FOLDER_ID) + PATH_SEPARATOR + QString::number(newlyCreatedNodeId);
...
void TreeViewLogic::onAddTagRequested() {
    ...
    QMetaObject::invokeMethod(m_dbManager, "addTag", Qt::BlockingQueuedConnection, Q_RETURN_ARG(int, newlyCreatedTagId),
...
void TreeViewLogic::onDeleteFolderRequested(const QModelIndex& index) {
    ...
    if (btn == QMessageBox::Yes) {
        ...
        NodeData node;
        QMetaObject::invokeMethod(m_dbManager, "getNode", Qt::BlockingQueuedConnection, Q_RETURN_ARG(NodeData, node),
                                  Q_ARG(int, id));
...
void TreeViewLogic::openFolder(int id) {
    NodeData target;
    QMetaObject::invokeMethod(m_dbManager, "getNode", Qt::BlockingQueuedConnection, Q_RETURN_ARG(NodeData, target),
                              Q_ARG(int, id));
