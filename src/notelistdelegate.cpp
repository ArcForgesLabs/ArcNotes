    ...
    connect(m_timeLine, &QTimeLine::finished, this, [this]() {
        ...
        if (!m_animationQueue.empty()) {
            ...
            for (const auto& id : std::as_const(a.first)) {
                const auto* noteListModel = static_cast<NoteListModel*>(m_view->model());
...
void NoteListDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (m_view->isPinnedNotesCollapsed()) {
        const auto* model = static_cast<NoteListModel*>(m_view->model());
...
QSize NoteListDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (m_view->isPinnedNotesCollapsed()) {
        ...
        if (noteListModel->hasPinnedNote() &&
            (noteListModel->isFirstPinnedNote(index) || noteListModel->isFirstUnpinnedNote(index))) {
            result.setHeight(result.height() + 25);
        }
    }
    int secondYOffset = 0;
    ...
    if (noteListModel->hasPinnedNote() && !m_view->isPinnedNotesCollapsed() &&
        noteListModel->isFirstUnpinnedNote(index)) {
...
void NoteListDelegate::paintBackground(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    const auto* model = static_cast<NoteListModel*>(m_view->model());
    if (model->hasPinnedNote() && model->isFirstPinnedNote(index) &&
        static_cast<NoteListView*>(m_view)->isPinnedNotesCollapsed()) {
    ...
    if (static_cast<NoteListView*>(m_view)->isDragging() && !isPinned &&
        !static_cast<NoteListView*>(m_view)->isDraggingInsidePinned()) {
    ...
    if (m_animatedIndexes.contains(index)) {
        if (m_state == NoteListState::MoveIn) {
            if (model->hasPinnedNote() && (model->isFirstPinnedNote(index) || model->isFirstUnpinnedNote(index))) {
                painter->drawPixmap(QRect{option.rect.x(), option.rect.y() + bufferSize.height() - rowHeight + 25,
                                          option.rect.width(), rowHeight},
                                    buffer, QRect{0, bufferSize.height() - rowHeight, option.rect.width(), rowHeight});
            } else {
                painter->drawPixmap(QRect{option.rect.x(), option.rect.y() + bufferSize.height() - rowHeight,
                                          option.rect.width(), rowHeight},
                                    buffer, QRect{0, bufferSize.height() - rowHeight, option.rect.width(), rowHeight});
            }
        } else {
            if (model->hasPinnedNote() && (model->isFirstPinnedNote(index) || model->isFirstUnpinnedNote(index))) {
                painter->drawPixmap(
                    QRect{option.rect.x(), option.rect.y() + 25, option.rect.width(), option.rect.height()}, buffer,
...
void NoteListDelegate::paintLabels(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    if (m_animatedIndexes.contains(index)) {
        ...
        QFont titleFont =
            (option.state & QStyle::State_Selected) == QStyle::State_Selected ? m_titleSelectedFont : m_titleFont;
        ...
        double contentRectPosY =
            rowPosY + fmRectTitle.height() + fmRectDate.height() + note_list_constants::TOP_OFFSET_Y + yOffsets;
        ...
        if (m_isInAllNotes) {
            ...
            folderNameRectPosY = rowPosY + fmRectContent.height() + fmRectTitle.height() + fmRectDate.height() +
                                 note_list_constants::TOP_OFFSET_Y + yOffsets;
        ...
        auto drawStr = [&bufferPainter](double posX, double posY, double width, double height, QColor color,
        ...
        if (m_isInAllNotes) {
            bufferPainter.drawImage(QRect(rowPosX + note_list_constants::LEFT_OFFSET_X,
                                          folderNameRectPosY + note_list_constants::DESC_FOLDER_SPACE, 16, 16),
                                    m_folderIcon);
            drawStr(folderNameRectPosX, folderNameRectPosY, folderNameRectWidth, folderNameRectHeight, m_contentColor,
                    titleFont, parentName);
        }
        drawStr(contentRectPosX, contentRectPosY, contentRectWidth, contentRectHeight, m_contentColor, titleFont,
                content);
        painter->setRenderHints(QPainter::Antialiasing | QPainter::SmoothPixmapTransform);
        int rowHeight;
        if (m_animatedIndexes.contains(index)) {
            ...
            if (noteListModel->isFirstPinnedNote(index)) {
                ...
                if (m_view->isPinnedNotesCollapsed()) {
                    painter->drawText(QRect(headerRect.right() - 25, headerRect.y() + 5, 16, 16),
                                      QStringLiteral("\uf054"));  // fa-chevron-right
                } else {
                    painter->drawText(QRect(headerRect.right() - 25, headerRect.y() + 5, 16, 16),
                                      QStringLiteral("\uf078"));  // fa-chevron-down
                }
                ...
                QRect headerRect(option.rect.x() + (note_list_constants::LEFT_OFFSET_X / 2),
        ...
        if (noteListModel->isFirstPinnedNote(index)) {
            QRect headerRect(rowPosX + (note_list_constants::LEFT_OFFSET_X / 2), rowPosY,
                             option.rect.width() - (note_list_constants::LEFT_OFFSET_X / 2), 25);
#ifdef __APPLE__
            int iconPointSizeOffset = 0;
#else
        ...
        auto drawStr = [painter](double posX, double posY, double width, double height, QColor color, const QFont& font,
                                 const QString& str) {
            QRectF rect(posX, posY, width, height);
        ...
        drawStr(dateRectPosX, dateRectPosY, dateRectWidth, dateRectHeight, m_dateColor, m_dateFont, date);
        if (isInAllNotes()) {
            painter->drawImage(QRect(rowPosX + note_list_constants::LEFT_OFFSET_X,
                                     folderNameRectPosY + note_list_constants::DESC_FOLDER_SPACE, 16, 16),
                               m_folderIcon);
            drawStr(folderNameRectPosX, folderNameRectPosY, folderNameRectWidth, folderNameRectHeight, m_contentColor,
                    titleFont, parentName);
...
#endif
        painter->setFont(font_loader::loadFont("Font Awesome 6 Free Solid", "", 14 + iconPointSizeOffset));
        painter->drawText(iconRect, QStringLiteral("\uf111"));  // fa-circle
