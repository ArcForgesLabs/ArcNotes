...
void NoteListView::animateAddedRow(const QModelIndexList& indexes) {
    ...
    if (delegate != nullptr) delegate->setState(NoteListState::Insert, indexes);
}
...
void NoteListView::mousePressEvent(QMouseEvent* e) {
    ...
    const auto* noteListModel = static_cast<NoteListModel*>(this->model());
...
void NoteListView::startDrag(Qt::DropActions supportedActions) {
...
#ifdef __APPLE__
        QFont displayFont(QFont(QStringLiteral("SF Pro Text")).exactMatch() ? QStringLiteral("SF Pro Text")
                                                                            : QStringLiteral("Roboto"));
#elif _WIN32
        QFont displayFont(QFont(QStringLiteral("Segoe UI")).exactMatch() ? QStringLiteral("Segoe UI")
                                                                         : QStringLiteral("Roboto"));
#else
    ...
    m_isDraggingPinnedNotes = false;
    m_isDraggingPinnedNotes = std::any_of(indexes.cbegin(), indexes.cend(), [](const QModelIndex& index) {
...
void NoteListView::setupSignalsSlots() {
    ...
    connect(this, &NoteListView::viewportEntered, this, [this]() {
        if (model() && model()->rowCount() > 1) {
            auto* delegate = static_cast<NoteListDelegate*>(itemDelegate());
            if (delegate) delegate->setHoveredIndex(QModelIndex());
...
void NoteListView::addNotesToTag(const QSet<int>& notesId, int tagId) {
...
void NoteListView::removeNotesFromTag(const QSet<int>& notesId, int tagId) {
    for (const auto& id : std::as_const(notesId)) {
        const auto* noteListModel = static_cast<NoteListModel*>(this->model());
        if (noteListModel != nullptr) {
            auto index = noteListModel->getNoteIndex(id);
